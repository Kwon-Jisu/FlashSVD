...
def main():
    import argparse
    ap = argparse.ArgumentParser("ModernBERT explicit-SVD long-context profiling")
    ...
    # Ranks for compression testing (head-wise and FFN)
    RANK_ATTN = int(args.rank_attn)
    RANK_FFN  = int(args.rank_ffn)

    cfg = AutoConfig.from_pretrained(MODE_DIR := args.model_dir, trust_remote_code=True)
    ...
    # Warmup to finish any kernel JIT/tuning then reset peak stats
    ...
    # Comprehensive memory and latency measurement
    ...
    print(
        f"Explicit SVD | L={args.seq_len} bs={args.batch_size} | acc={full_acc:.4f} | "
