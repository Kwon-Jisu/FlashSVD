...
def _build_loader(tokenizer, seq_len=128, batch_size=8):
    # Hardcode IMDB dataset for long-context profiling
    raw = load_dataset("imdb", split="test")
    def tok(b): return tokenizer(b["text"], padding="max_length", truncation=True, max_length=seq_len)
    ds = raw.map(tok, batched=True, remove_columns=[c for c in raw.column_names if c not in ("label",)])
    ds.set_format("torch")
...
@torch.no_grad()
def acc_peak_time(model_svd, loader, device, use_mask=True, max_steps: int = 0):
    metric = load_metric("accuracy")
    ...
    return metric.compute()["accuracy"], peak_mib, elapsed_s * 1000.0 / max(1, steps), (tokens / elapsed_s) if tokens else 0.0


@torch.no_grad()
def warmup(model, loader, device, steps: int = 3):
    if steps <= 0:
        return
    it = iter(loader)
    ran = 0
...
def main():
    import argparse
    ap = argparse.ArgumentParser("ModernBERT FlashSVD long-context profiling")
    ap.add_argument("--model-dir", default=MODEL_DIR)
    ap.add_argument("--seq-len", type=int, default=2048)
    ap.add_argument("--batch-size", type=int, default=16)
    ap.add_argument("--rank-attn", type=int, default=64//4)
    ap.add_argument("--rank-ffn", type=int, default=768//4)
    ap.add_argument("--ffn-pinned", type=int, default=1, help="1: use pinned FFN config; 0: autotune")
    ...
    RANK_ATTN = int(args.rank_attn)
    RANK_FFN  = int(args.rank_ffn)

    cfg = AutoConfig.from_pretrained(args.model_dir, trust_remote_code=True)
    cfg._attn_implementation = "sdpa"

    dense = AutoModelForSequenceClassification.from_pretrained(args.model_dir, config=cfg, trust_remote_code=True).to(device).eval()
    tok = AutoTokenizer.from_pretrained(args.model_dir, trust_remote_code=True)
    loader = _build_loader(tok, seq_len=args.seq_len, batch_size=args.batch_size)
    ...
    USE_FFN_PINNED = bool(args.ffn_pinned)   # default True (1)
    ...
    # Warmup to finish JIT/autotune then reset peak stats
    warmup(svd, loader, device, steps=args.warmup_steps)
    torch.cuda.reset_peak_memory_stats()

    acc = quick_check(svd, loader, device)
    print(f"[Sanity] Model accuracy on 3 batches: {acc:.4f}")

    # Comprehensive memory and latency measurement (steady-state after warmup)
    full_acc, peak_lr, latency_ms, tok_per_s = acc_peak_time(svd, loader, device, use_mask=True, max_steps=args.max_steps)
    ...
    print(
        f"FlashSVD     | L={args.seq_len} bs={args.batch_size} | acc={full_acc:.4f} | "
        f"peak={peak_lr:6.1f} MiB | real={real_peak_mib:6.1f} MiB | transient={transient_mib:6.1f} MiB | "
        f"{latency_ms:6.1f} ms/b | {tok_per_s/1e6:5.2f}M tok/s"
    )
