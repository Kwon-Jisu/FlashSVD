...
MODEL_DIR = "../model/modernbert-base-imdb"
...
@torch.no_grad()
def acc_peak_time(model, loader, device, use_mask=True, max_steps: int = 0):
    metric = load_metric("accuracy")
...
def main():
    import argparse
    ap = argparse.ArgumentParser("ModernBERT dense long-context profiling")
    ap.add_argument("--model-dir", default=MODEL_DIR)
    ap.add_argument("--seq-len", type=int, default=2048)
    ap.add_argument("--batch-size", type=int, default=16)
    ap.add_argument("--warmup-steps", type=int, default=3)
    ap.add_argument("--max-steps", type=int, default=0, help="Limit number of evaluation steps (0=all)")
    args = ap.parse_args()
    ...
    warmup(custom, loader, device, steps=args.warmup_steps)
    torch.cuda.reset_peak_memory_stats()
    ...
    full_acc, peak_lr, latency_ms, tok_per_s = acc_peak_time(custom, loader, device, use_mask=True, max_steps=args.max_steps)
    ...
    print(
        f"Dense        | L={args.seq_len} bs={args.batch_size} | acc={full_acc:.4f} | "
