import os, math, time, platform, inspect, types
import torch
...
Env toggles:
  ...
  ASVD=1|0                    # enable low-rank (Pk,Pv) caching in decode
    ...
    def rec(obj) -> int:
        ...
        if total == 0:
            try:
                ...
                pass
        return total
    return rec(past_key_values) / (1024.0 ** 2)
...
class SVDLlamaBlock(nn.Module):
    ...
    def __init__(self, hf_layer: nn.Module, cfg,
        ...
        self.asvd_enabled = os.getenv("ASVD", "0") == "1"
    ...
    def forward(self, hidden_states, attention_mask=None, past_key_value=None,
        ...
        if not use_cache:
            ...
            if self.asvd_enabled:
                ...
                # Non-ASVD cache path (HF cache object or tuples)
                k_raw = self._proj_per_head(x_trim, self.k_Us, self.k_V)     # [B,Hk,T,dh]
                v_raw = self._proj_per_head(x_trim, self.v_Us, self.v_V)
                q, k_rot = self._apply_rope(q, k_raw, pos_ids, position_embeddings=position_embeddings)
                ...
                if hasattr(past_key_value, "update"):
                    ...
                    try:
                        k_seq, v_seq = past_key_value.update(k_rot, v_raw, layer_idx=li, cache_position=cache_position)
                    except TypeError:
                        try:
                    ...
                    if isinstance(past_key_value, (tuple, list)) and len(past_key_value) == 2:
                        k_seq = torch.cat([past_key_value[0], k_rot], dim=-2)
...
def profile_prefill_and_decode(model, tok: AutoTokenizer, device: str,
                               ...
                               batch_size: int = 1,
                               rank_kv: int = 128):
    asvd_enable = os.getenv("ASVD", "0") == "1"
    ...
    final_kv_est = (_estimate_asvd_kv_cache_mib(model.config, batch_size=batch_size,
                    ...
                    if asvd_enable else
                    _estimate_kv_cache_mib(model.config, batch_size=batch_size,
                                           seq_len=prompt_len + decode_tokens,
